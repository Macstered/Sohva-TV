...
For **0.1.0-beta.9**, Android build **10**.
...
1. Download `sohva-tv-0.1.0-beta.9.apk` from the developer's supplied package
...
```powershell
adb devices -l
adb -s YOUR_DEVICE_SERIAL install -r .\sohva-tv-0.1.0-beta.9.apk
```
...
```powershell
Get-FileHash .\sohva-tv-0.1.0-beta.9.apk -Algorithm SHA256
```
